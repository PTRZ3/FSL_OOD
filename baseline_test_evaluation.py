...
from siamese_resnet import BasicBlock, Siamese_ResNet      #siamese
#from baseline_resnet import BasicBlock, Baseline_ResNet   #siamese
...
def test_evaluation(model, k_shot=5, n_way=6, n_query_img=1000, seen_classes=list(range(6)), ood_classes=[8, 9],
    ...
    for i, test_batch in enumerate(test_dataloader, 0):
        ...
        #support_emb = model(test_support_image.cuda())[0].detach().cpu().numpy()   #siamese
        support_emb = model(test_support_image.cuda(), test_support_image.cuda())[0].detach().cpu().numpy()     #siamese
        support_class = test_support_class.numpy().squeeze()

        # query_emb, query_ideal_logits = model(test_query_image[0].unsqueeze(0).cuda())       #siamese
        query_emb = model(test_query_image[0].unsqueeze(0).cuda(), test_query_image[0].unsqueeze(0).cuda())[0]    #siamese
        query_emb = query_emb.detach().cpu().numpy()
        # query_ideal_ood_score = nn.functional.softmax(query_ideal_logits, dim=1).detach().cpu().numpy().squeeze().max()   #siamese
        ...
        # q_ideal_ood_score.append(1 - query_ideal_ood_score)  #siamese
    ...
    #test_metrics["auroc_query_is_ood_ideal"] = show_auroc(q_is_ood, q_ideal_ood_score, display=plot)  #siamese
    #print("auroc of query image ideal ood score (pre-trained linear layer): ", test_metrics["auroc_query_is_ood_ideal"])  #siamese
...
if __name__ == "__main__":
    ...
    ### load model ###
    #model = Baseline_ResNet(BasicBlock, [2,2,2,2], num_classes=6).cuda()  #siamese
    #model.load_state_dict(torch.load("./stat_dict/baseline_pretrain_best.pth"))   #siamese
    model = Siamese_ResNet(BasicBlock, [2,2,2,2]).cuda()  #siamese
    model.load_state_dict(torch.load("./stat_dict/siamese_without_oe_best_rotation.pth"))   #siamese
    ...
    wandb.init(mode="online",
               ...
               # project='Testing Evaluation - Baseline_Max_Softmax',  #siamese
               project = 'Testing Evaluation - Siamese_Max_Softmax',   #siamese
    ...
    wandb.log({
        ...
        #"auroc_query_is_ood_ideal": test_metrics['auroc_query_is_ood_ideal'],  #siamese
