...
def test_evaluation(train_class_center, model, k_shot=5, n_way=6, n_query_img=1000, seen_classes=list(range(6)), ood_classes=[8, 9],
    ...
    for i, test_batch in enumerate(test_dataloader, 0):
        ...
        support_class_center = torch.stack(
            [s_emb[(i * k_shot):(i * k_shot + k_shot)].mean(axis=0) for i in range(n_way)]).cpu().detach()
        q_ood_score_sc.append(min(F.pairwise_distance(q_emb[:n_way].cpu().detach(),
                                                      support_class_center)).item())  # use min distance to support class center
    ...
    test_diff_seen_dist = [pair_dist[i] for i in range(len(pair_dist)) if
                           (pair_is_diff[i] == 1 and pair_is_ood[i] == 0)]
    ...
    test_metrics["auroc_pair_is_diff"] = show_auroc(pair_is_diff, pair_dist, display = plot)

    print("auroc of test pair_is_ood")
    test_metrics["auroc_pair_is_ood"] = show_auroc(pair_is_ood, pair_dist, display = plot)
    ...
    print("auroc of query image ood score")
    ...
    test_metrics["auroc_query_is_ood_sc"] = show_auroc(q_is_ood, q_ood_score_sc, display = plot)

    print("auroc of query image ideal ood score (distance to class center)")
    test_metrics["auroc_query_is_ood_ideal"] = show_auroc(q_is_ood, q_ideal_ood_score, display = plot)

    print("accuracy of query image from seen classes:", metrics.accuracy_score(q_class[0::2], q_pred_class[0::2]))
...
if __name__ == "__main__":
    ...
    # model.load_state_dict(torch.load("./stat_dict/siamese_with_oe_continue.pth"))
    ...
    wandb.init(mode="online",
               ...
               project='Testing Evaluation - Siamese_without_OE',
